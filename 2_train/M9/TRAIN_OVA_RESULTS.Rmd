...
model_type <- 'fa_5pct_wt1_lga_comb'
...
## runway features
runway_keep <- names(X)[which(grepl("rwycnfg",names(X)) == TRUE)]

## important features
important_keep <- unique(rate_var_imp$variable)
...
X_train <- cbind(
  X_probs
  , X[,unique(c(runway_keep, important_keep))]
)
