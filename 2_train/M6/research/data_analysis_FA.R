...
## oag vars
oag_features <- names(X)[which(grepl("oag", names(X)) == TRUE)]
rem_vars <- c(oag_features, time_features)
...
## are values of predictor significantly different between rate and non-rate hours?
rate_data <- cbind(Y_rate, X_rate)
sig_analysis <- c()
for(var in names(X_rate)){
  rate_data$dum <- rate_data[,var]
  test <- t.test(formula = dum ~ Y_rate, data = rate_data)
  temp <- data.frame(
    "var" = var
    , "rate_mean" = as.numeric(test$estimate[2])
    , "non_rate_mean" = as.numeric(test$estimate[1])
    , "t_value" = test$statistic
    , "p_value" = test$p.value
  )
  sig_analysis <- rbind(sig_analysis, temp)
}
## yes -- we see what we would expect: lower ceiling and vis; less VFR
## interestingly, there are no 28 rates called during September
## removing OAG features decreases false alarms, but also decreases hits
